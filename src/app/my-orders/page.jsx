...
//                                 <span className="flex-1 text-sm text-[#2C1810] dark:text-[#F5E6C0]">
...
import { useEffect, useState } from 'react'
import { useRouter } from 'next/navigation'
import RegularOrders from '@/components/my-orders/RegularOrders'
import CustomOrders from '@/components/my-orders/CustomOrders'
...
export default function OrdersPage() {
  ...
  return (
    <div className="pt-[72px] min-h-screen bg-[#FDF6EC] dark:bg-[#1A0F0A] transition-colors duration-300">
      ...
      {/* Header */}
      <div className="bg-gradient-to-br from-[#2C1810] to-[#5C3317] py-12 px-6 text-center">
        <span className="block text-[#C9A84C] text-xs font-bold tracking-[4px] uppercase mb-2">My Account</span>
        <h1 className="text-3xl sm:text-4xl font-black text-white" style={{ fontFamily: 'Playfair Display, serif' }}>
          My <em className="italic text-[#C9A84C]">Orders</em>
        </h1>
      </div>
      ...
      <div className="max-w-5xl mx-auto px-4 py-8">
        ...
        {activeTab === 'custom' && (
          <CustomOrders
            ...
            setExpandedCustom={setExpandedCustom}
          />
        )}
      </div>
    </div>
