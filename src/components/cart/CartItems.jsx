    ...
    <div className="bg-white dark:bg-[#2C1810] border border-yellow-700/20 overflow-hidden rounded-md">
      {cart.map((item, i) => (
        <div
          ...
          className={`flex flex-col sm:flex-row items-start sm:items-center gap-3 sm:gap-4 p-4 sm:p-5 ${
            i !== cart.length - 1
              ? 'border-b border-yellow-700/15 dark:border-yellow-700/25'
              : ''
          }`}
        >
          {/* Product Image */}
          <div className="w-12 h-12 sm:w-14 sm:h-14 flex items-center justify-center bg-[#FDF6EC] dark:bg-[#1A0F0A] flex-shrink-0 rounded-md overflow-hidden">
            {item.image ? (
              <Image
                src={item.image}
                alt={item.name}
